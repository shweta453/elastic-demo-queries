...
import com.elasticsearch.git.elasticdemoqueries.model.WSUsersResponse;
...
public class UserIndexSearchService {
    ...
    public WSUsersResponse wildcardSearch(String query, Integer offset, Integer limit) {
        ...
        queryBuilders.should(QueryBuilders.wildcardQuery("phrase","*" + query + "*"));
        sourceBuilder.query(queryBuilders);
        searchRequest.source(sourceBuilder);
        SearchResponse searchResponse = highLevelRestClient.postSearchQueries(searchRequest);
        log.info("Search JSON query: {}", searchRequest.source().toString());
    ...
    public WSUsersResponse regExpSearch(String query, Integer offset, Integer limit) {
        ...
        sourceBuilder.from(offset);
        sourceBuilder.size(limit);
        sourceBuilder.query(QueryBuilders.regexpQuery("siblings", "a[a-z]*h"));
        searchRequest.source(sourceBuilder);
        SearchResponse searchResponse = highLevelRestClient.postSearchQueries(searchRequest);
        log.info("Search JSON query: {}", searchRequest.source().toString());
        return extractUserResponse(searchResponse);
